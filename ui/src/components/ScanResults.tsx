      ...
      <div className="bg-slate-800 rounded-lg p-4 flex items-center justify-between">
        ...
        {metadata && (
          <div className="text-right text-sm text-slate-400">
            <div>
              Game: <span className="text-slate-300">{metadata.game}</span>
            </div>
            <div>
              Lines:{' '}
              <span className="text-slate-300">{metadata.linesProcessed.toLocaleString()}</span>
            </div>
            {metadata.durationMs !== undefined && (
              <div>
                Time: <span className="text-slate-300">{metadata.durationMs}ms</span>
              </div>
            )}
          </div>
        )}
      ...
      {issues.length === 0 ? (
        <div className="bg-green-900/30 border border-green-700 rounded-lg p-8 text-center">
          ...
          <p className="text-green-300/80">
            The crash log doesn't contain any known problematic patterns.
          </p>
        </div>
      ) : (
        <div className="space-y-3">
          <h3 className="text-lg font-semibold text-slate-200">Issues Found ({issues.length})</h3>
